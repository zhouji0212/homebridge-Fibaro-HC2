...
export class GetFunctions {
  ...
  constructor(hapCharacteristic, platform) {
    ...
    this.getFunctionsMapping = new Map([
      ...
      [
        new hapCharacteristic.Brightness().UUID,
        { function: this.getBrightness, delay: 0 },
      ],
      [
        new hapCharacteristic.PositionState().UUID,
        { function: this.getPositionState, delay: 0 },
      ],
      [
        new hapCharacteristic.CurrentPosition().UUID,
        { function: this.getCurrentPosition, delay: 0 },
      ],
      [
        new hapCharacteristic.TargetPosition().UUID,
        { function: this.getCurrentPosition, delay: 0 },
      ], // Manage the same as currentPosition
      [
        new hapCharacteristic.CurrentHorizontalTiltAngle().UUID,
        { function: this.getCurrentTiltAngle, delay: 0 },
      ],
      [
        new hapCharacteristic.TargetHorizontalTiltAngle().UUID,
        { function: this.getCurrentTiltAngle, delay: 0 },
      ],
      [
        new hapCharacteristic.MotionDetected().UUID,
        { function: this.getBool, delay: 0 },
      ],
      [
        new hapCharacteristic.CurrentTemperature().UUID,
        { function: this.getFloat, delay: 0 },
      ],
      [
        new hapCharacteristic.TargetTemperature().UUID,
        { function: this.getTargetTemperature, delay: 0 },
      ],
      [
        new hapCharacteristic.CurrentRelativeHumidity().UUID,
        { function: this.getFloat, delay: 0 },
      ],
      [
        new hapCharacteristic.ContactSensorState().UUID,
        { function: this.getContactSensorState, delay: 0 },
      ],
      [
        new hapCharacteristic.LeakDetected().UUID,
        { function: this.getLeakDetected, delay: 0 },
      ],
      [
        new hapCharacteristic.SmokeDetected().UUID,
        { function: this.getSmokeDetected, delay: 0 },
      ],
      [
        new hapCharacteristic.CarbonMonoxideDetected().UUID,
        { function: this.getCarbonMonoxideDetected, delay: 0 },
      ],
      [
        new hapCharacteristic.CarbonMonoxideLevel().UUID,
        { function: this.getFloat, delay: 0 },
      ],
      [
        new hapCharacteristic.CarbonMonoxidePeakLevel().UUID,
        { function: this.getFloat, delay: 0 },
      ],
      [
        new hapCharacteristic.CurrentAmbientLightLevel().UUID,
        { function: this.getFloat, delay: 0 },
      ],
      [
        new hapCharacteristic.OutletInUse().UUID,
        { function: this.getOutletInUse, delay: 0 },
      ],
      [
        new hapCharacteristic.LockCurrentState().UUID,
        {
          function: this.getLockCurrentState,
          delay: this.platform.config.LockCurrentStateDelay,
        },
      ],
      [
        new hapCharacteristic.LockTargetState().UUID,
        {
          function: this.getLockCurrentState,
          delay: this.platform.config.LockTargetStateDelay,
        },
      ], // Manage the same as currentState
      [
        new hapCharacteristic.CurrentHeatingCoolingState().UUID,
        { function: this.getCurrentHeatingCoolingState, delay: 0 },
      ],
      [
        new hapCharacteristic.TargetHeatingCoolingState().UUID,
        { function: this.getTargetHeatingCoolingState, delay: 0 },
      ],
      [
        new hapCharacteristic.TemperatureDisplayUnits().UUID,
        { function: this.getTemperatureDisplayUnits, delay: 0 },
      ],
      [new hapCharacteristic.Hue().UUID, { function: this.getHue, delay: 0 }],
      [
        new hapCharacteristic.Saturation().UUID,
        { function: this.getSaturation, delay: 0 },
      ],
      [
        new hapCharacteristic.CurrentDoorState().UUID,
        { function: this.getCurrentDoorState, delay: 0 },
      ],
      [
        new hapCharacteristic.TargetDoorState().UUID,
        { function: this.getCurrentDoorState, delay: 0 },
      ],
      [
        new hapCharacteristic.ObstructionDetected().UUID,
        { function: this.getObstructionDetected, delay: 0 },
      ],
      [
        new hapCharacteristic.BatteryLevel().UUID,
        { function: this.getBatteryLevel, delay: 0 },
      ],
      [
        new hapCharacteristic.ChargingState().UUID,
        { function: this.getChargingState, delay: 0 },
      ],
      [
    ...
    this.getCurrentSecuritySystemStateMapping = new Map([
      ...
      [
        "NightArmed",
        this.hapCharacteristic.SecuritySystemCurrentState.NIGHT_ARM,
      ],
      ["StayArmed", this.hapCharacteristic.SecuritySystemCurrentState.STAY_ARM],
      [
        "AlarmTriggered",
        this.hapCharacteristic.SecuritySystemCurrentState.ALARM_TRIGGERED,
      ],
    ]);
    this.getTargetSecuritySystemStateMapping = new Map([
      ...
      [
        "NightArmed",
        this.hapCharacteristic.SecuritySystemTargetState.NIGHT_ARM,
      ],
  ...
  returnValue(r, callback, characteristic) {
    ...
    else characteristic.updateValue(r);
  }
  // Boolean getter
  ...
  getFloat(callback, characteristic, service, IDs, properties) {
    ...
    if (service.floatServiceId) {
      this.platform.fibaroClient
        .getDeviceProperties(service.floatServiceId)
        .then((properties) => {
        ...
        .catch((err) => {
          console.log(
            "There was a problem getting value from: ",
            `${service.floatServiceId} - Err: ${err}`
          );
          callback(err, null);
        });
    } else {
  ...
  getPositionState(callback, characteristic, service, IDs, properties) {
    this.returnValue(
      this.hapCharacteristic.PositionState.STOPPED,
      callback,
      characteristic
    );
  }
  getCurrentPosition(callback, characteristic, service, IDs, properties) {
    ...
    if (
      ...
      r <= characteristic.props.maxValue
    ) {
      if (r == 99) r = 100;
      else if (r == 1) r = 0;
    } else {
  ...
  getTargetTemperature(callback, characteristic, service, IDs, properties) {
    this.returnValue(
      parseFloat(properties.targetLevel),
      callback,
      characteristic
    );
  }
  ...
  getLeakDetected(callback, characteristic, service, IDs, properties) {
    this.returnValue(
      properties.value == "true"
        ? this.hapCharacteristic.LeakDetected.LEAK_DETECTED
        : this.hapCharacteristic.LeakDetected.LEAK_NOT_DETECTED,
      callback,
      characteristic
    );
  }
  getSmokeDetected(callback, characteristic, service, IDs, properties) {
    this.returnValue(
      properties.value == "true"
        ? this.hapCharacteristic.SmokeDetected.SMOKE_DETECTED
        : this.hapCharacteristic.SmokeDetected.SMOKE_NOT_DETECTED,
      callback,
      characteristic
    );
  }
  getCarbonMonoxideDetected(
    ...
    this.returnValue(
      properties.value == "true"
        ? this.hapCharacteristic.CarbonMonoxideDetected.CO_LEVELS_ABNORMAL
        : this.hapCharacteristic.CarbonMonoxideDetected.CO_LEVELS_NORMAL,
      callback,
      characteristic
    );
  }
  getOutletInUse(callback, characteristic, service, IDs, properties) {
    this.returnValue(
      parseFloat(properties.power) > 1.0 ? true : false,
      callback,
      characteristic
    );
  }
  ...
  getCurrentHeatingCoolingState(
    ...
    if (service.operatingModeId) {
      // Operating mode is availble on Home Center
      this.platform.fibaroClient
        .getDeviceProperties(service.operatingModeId)
        .then((properties) => {
          switch (properties.mode) {
            case "0": // OFF
              this.returnValue(
                this.hapCharacteristic.CurrentHeatingCoolingState.OFF,
                callback,
                characteristic
              );
              break;
            case "1": // HEAT
              this.returnValue(
                this.hapCharacteristic.CurrentHeatingCoolingState.HEAT,
                callback,
                characteristic
              );
              break;
            case "2": // COOL
              this.returnValue(
                this.hapCharacteristic.CurrentHeatingCoolingState.COOL,
                callback,
                characteristic
              );
              break;
        ...
        .catch((err) => {
          this.platform.log(
            "There was a problem getting value from: ",
            `${service.operatingModeId} - Err: ${err}`
          );
          callback(err, null);
        });
    } else {
      if (this.platform.config.enablecoolingstatemanagemnt == "on") {
        ...
        // Fake simulated mode: always heat
        this.returnValue(
          this.hapCharacteristic.CurrentHeatingCoolingState.HEAT,
          callback,
          characteristic
      ...
      this.platform.fibaroClient
        ...
        .then((properties) => {
          switch (properties.mode) {
            case "0": // OFF
              this.returnValue(
                this.hapCharacteristic.TargetHeatingCoolingState.OFF,
                callback,
                characteristic
              );
              break;
            case "1": // HEAT
              this.returnValue(
                this.hapCharacteristic.TargetHeatingCoolingState.HEAT,
                callback,
                characteristic
              );
              break;
            case "2": // COOL
              this.returnValue(
                this.hapCharacteristic.TargetHeatingCoolingState.COOL,
                callback,
                characteristic
              );
              break;
            case "10": // AUTO
              this.returnValue(
                this.hapCharacteristic.TargetHeatingCoolingState.AUTO,
                callback,
                characteristic
              );
              break;
  ...
  getTemperatureDisplayUnits(
    ...
    this.returnValue(
      this.hapCharacteristic.TemperatureDisplayUnits.CELSIUS,
      callback,
      characteristic
    );
  }
  getHue(callback, characteristic, service, IDs, properties) {
    if (properties.color)
      this.returnValue(
        Math.round(
          this.updateHomeKitColorFromHomeCenter(properties.color, service).h
        ),
        callback,
        characteristic
      );
  ...
  getSaturation(callback, characteristic, service, IDs, properties) {
    if (properties.color)
      this.returnValue(
        Math.round(
          this.updateHomeKitColorFromHomeCenter(properties.color, service).s
        ),
        callback,
        characteristic
      );
  ...
  getCurrentDoorState(callback, characteristic, service, IDs, properties) {
    this.returnValue(
      properties.state == "Closed"
        ? this.hapCharacteristic.CurrentDoorState.CLOSED
        : this.hapCharacteristic.CurrentDoorState.OPEN,
      callback,
      characteristic
    );
  }
  ...
  getSecuritySystemState(
    ...
    if (
      characteristic.UUID ==
      new this.hapCharacteristic.SecuritySystemCurrentState().UUID
    ) {
      r = this.getCurrentSecuritySystemStateMapping.get(
        securitySystemStatus.value
      );
    } else if (
      characteristic.UUID ==
      new this.hapCharacteristic.SecuritySystemTargetState().UUID
    ) {
      r = this.getTargetSecuritySystemStateMapping.get(
        securitySystemStatus.value
      );
    }
    if (r == undefined)
