...
export class ShadowAccessory {
  ...
  constructor(
    ...
    for (let i = 0; i < services.length; i++) {
      ...
      // DEVICE_ID-VIRTUAL_BUTTON_ID-RGB_MARKER-OPERATING_MODE_ID-PLUGIN_MARKER
    }
  }

  initAccessory() {
    const properties = this.device.properties || {};
    const manufacturer = (properties.zwaveCompany || "IlCato").replace(
      "Fibargroup",
      "Fibar Group"
    );
    this.accessory
      .getService(this.hapService.AccessoryInformation)
      .setCharacteristic(this.hapCharacteristic.Manufacturer, manufacturer)
      .setCharacteristic(
        this.hapCharacteristic.Model,
        `${this.device.type || "HomeCenterBridgedAccessory"}`
      )
      .setCharacteristic(
        this.hapCharacteristic.SerialNumber,
  ...
  removeNoMoreExistingServices() {
    for (let t = 0; t < this.accessory.services.length; t++) {
      ...
      for (let s = 0; s < this.services.length; s++) {
        ...
        if (
          this.accessory.services[t].displayName == undefined ||
          this.services[s].controlService.displayName ==
            this.accessory.services[t].displayName
        ) {
          found = true;
          break;
        }
      ...
      if (
        !found &&
        this.accessory.services[t].UUID !==
          "0000003E-0000-1000-8000-0026BB765291"
      ) {
        // Accessory Information must be present
  ...
  addNewServices(platform) {
    for (let s = 0; s < this.services.length; s++) {
      ...
      let serviceExists = this.accessory.getService(
      ...
      if (!serviceExists) {
        ...
        for (let i = 0; i < service.characteristics.length; i++) {
          ...
          if (
            characteristic.UUID ==
            new this.hapCharacteristic.CurrentAmbientLightLevel().UUID
          ) {
          ...
          if (
            characteristic.UUID ==
            new this.hapCharacteristic.CurrentTemperature().UUID
          ) {
            characteristic.props.minValue = -50;
          }
          platform.bindCharacteristicEvents(
            characteristic,
  ...
  registerUpdateAccessory(isNewAccessory, api) {
    ...
    else api.updatePlatformAccessories([this.accessory]);
  ...
  static createShadowAccessory(
    ...
    switch (device.type) {
      ...
      case "com.fibaro.FGWD111":
        ...
        ss = [new FanService(controlService, controlCharacteristics)];
        break;
      ...
      case "com.fibaro.baseShutter":
        ss = [
          new FanService(new hapService.GarageDoorOpener(device.name), [
            hapCharacteristic.CurrentDoorState,
            hapCharacteristic.TargetDoorState,
            hapCharacteristic.ObstructionDetected,
          ]),
        ];
        break;
      ...
      case "com.fibaro.motionSensor":
        ss = [
          new FanService(new hapService.MotionSensor(device.name), [
      ...
      case "com.fibaro.temperatureSensor":
        ss = [
          new FanService(new hapService.TemperatureSensor(device.name), [
      ...
      case "com.fibaro.humiditySensor":
        ss = [
          new FanService(new hapService.HumiditySensor(device.name), [
      ...
      case "com.fibaro.satelZone":
        ss = [
          new FanService(new hapService.ContactSensor(device.name), [
      ...
      case "com.fibaro.floodSensor":
        ss = [
          new FanService(new hapService.LeakSensor(device.name), [
      ...
      case "com.fibaro.smokeSensor":
        ss = [
          new FanService(new hapService.SmokeSensor(device.name), [
      ...
      case "com.fibaro.FGCD001":
        ss = [
          new FanService(new hapService.CarbonMonoxideSensor(device.name), [
            hapCharacteristic.CarbonMonoxideDetected,
            hapCharacteristic.CarbonMonoxideLevel,
            hapCharacteristic.CarbonMonoxidePeakLevel,
            hapCharacteristic.BatteryLevel,
          ]),
        ];
        break;
      case "com.fibaro.lightSensor":
        ss = [
          new FanService(new hapService.LightSensor(device.name), [
      ...
      case "com.fibaro.FGWOEF011":
        ss = [
          new FanService(new hapService.Outlet(device.name), [
            hapCharacteristic.On,
            hapCharacteristic.OutletInUse,
          ]),
        ];
        break;
      ...
      case "com.fibaro.gerda":
        ss = [
          new FanService(new hapService.LockMechanism(device.name), [
            hapCharacteristic.LockCurrentState,
            hapCharacteristic.LockTargetState,
          ]),
        ];
      ...
      case "com.fibaro.com.fibaro.thermostatHorstmann":
        ...
        break;
      case "virtual_device":
        let pushButtonServices: Array<FanService> = new Array();
        let pushButtonService: FanService;
        for (let r = 0; r < device.properties.rows.length; r++) {
          if (device.properties.rows[r].type == "button") {
            for (
              let e = 0;
              e < device.properties.rows[r].elements.length;
              e++
            ) {
              pushButtonService = new FanService(
                new hapService.Switch(
                  device.properties.rows[r].elements[e].caption
                ),
                [hapCharacteristic.On]
              );
              pushButtonService.controlService.subtype =
                device.id + "-" + device.properties.rows[r].elements[e].id;
              pushButtonServices.push(pushButtonService);
            }
          }
        }
        if (pushButtonServices.length > 0) ss = pushButtonServices;
        break;
      ...
      case "com.fibaro.FGRGBW442CC":
        let service = {
          ...
          characteristics: [
            hapCharacteristic.On,
            hapCharacteristic.Brightness,
            hapCharacteristic.Hue,
            hapCharacteristic.Saturation,
          ],
        };
      ...
      case "com.fibaro.logitechHarmonyActivity":
        ...
        ss = [new FanService(controlService, [hapCharacteristic.On])];
        break;
      default:
        break;
    }
    ...
    if (device.interfaces && device.interfaces.includes("battery")) {
      ss.push(
        new FanService(new hapService.BatteryService(device.name), [
          hapCharacteristic.BatteryLevel,
          hapCharacteristic.ChargingState,
          hapCharacteristic.StatusLowBattery,
        ])
      );
    }

    return new ShadowAccessory(
      device,
      ss,
      hapAccessory,
      hapService,
  ...
  static createShadowSecuritySystemAccessory(
    device,
    hapAccessory,
    hapService,
    hapCharacteristic,
    ...
    let service = new FanService(
      new hapService.SecuritySystem("FibaroSecuritySystem"),
      [
        hapCharacteristic.SecuritySystemCurrentState,
        hapCharacteristic.SecuritySystemTargetState,
      ]
    );
  ...
  static createShadowGlobalVariableSwitchAccessory(
    device,
    hapAccessory,
    hapService,
    hapCharacteristic,
    ...
    let service = new FanService(new hapService.Switch(device.name), [
      hapCharacteristic.On,
    ]);
