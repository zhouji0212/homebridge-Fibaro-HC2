...
import { pluginName, platformName, ShadowAccessory } from "./shadows";
...
class Config {
  ...
  AcFanHighLevel?: string;
  AcFanMidLevel?: string;
  AcFanLowLevel?: string;
  constructor() {
    this.name = "";
    this.host = "";
    this.username = "";
...
class FibaroHC2 {
  ...
  config: Config;
  api: any;
  accessories: Map<string, any>; //虚拟设备列表
  ...
  constructor(
    ...
    if (!config) {
      this.log(
        ...
        "cannot find configuration for the plugin"
      );
      return;
    }
    let pollerPeriod = this.config.pollerperiod
      ? parseInt(this.config.pollerperiod)
      : defaultPollerPeriod;
    if (isNaN(pollerPeriod) || pollerPeriod < 0 || pollerPeriod > 100)
      pollerPeriod = defaultPollerPeriod;
    if (
      this.config.securitysystem == undefined ||
      (this.config.securitysystem != "enabled" &&
    ...
    if (this.config.IFTTTmakerkey == undefined) this.config.IFTTTmakerkey = "";
    if (
      this.config.enableIFTTTnotification == undefined ||
      this.config.enableIFTTTnotification == ""
    )
    ...
    //TODO:Config Add
    if (this.config.AcFanHighLevel == undefined) {
      this.config.AcFanHighLevel = "100";
    }
    if (this.config.AcFanMidLevel == undefined) {
      this.config.AcFanMidLevel = "50";
    }
    if (this.config.AcFanLowLevel == undefined) {
      this.config.AcFanLowLevel = "10";
    }
  ...
  LoadAccessories(devices) {
    ...
    devices.map((s, i, a) => {
      if (s.visible == true && s.name.charAt(0) != "_") {
        ...
        this.addAccessory(
          ShadowAccessory.createShadowAccessory(
            s,
            siblings,
            Accessory,
            Service,
    ...
    if (this.config.securitysystem == "enabled") {
      ...
      let sa = ShadowAccessory.createShadowSecuritySystemAccessory(
        device,
        Accessory,
        Service,
        Characteristic,
    ...
    if (
      ...
      for (let i = 0; i < globalVariables.length; i++) {
        ...
        let sa = ShadowAccessory.createShadowGlobalVariableSwitchAccessory(
          device,
          Accessory,
          Service,
          Characteristic,
    ...
    if (this.poller) this.poller.poll();
  }

  addAccessory(shadowAccessory) {
    if (shadowAccessory == undefined) return;
    ...
    if (
      this.config.securitysystem == "enabled" &&
      shadowAccessory.isSecuritySystem
    ) {
  ...
  bindCharacteristicEvents(characteristic, service) {
    ...
    if (!service.isVirtual) {
      ...
      if (service.HSBValue != undefined) propertyChanged = "valueandcolor";
      if (service.operatingModeId != undefined) {
        if (
          characteristic.UUID ==
            new Characteristic.CurrentHeatingCoolingState().UUID ||
          characteristic.UUID ==
            new Characteristic.TargetHeatingCoolingState().UUID
        ) {
          propertyChanged = "mode";
        }
      }
      if (
        service.UUID == Service.WindowCovering.UUID &&
        characteristic.UUID ==
          new Characteristic.CurrentHorizontalTiltAngle().UUID
      ) {
      ...
      if (
        service.UUID == Service.WindowCovering.UUID &&
        characteristic.UUID ==
          new Characteristic.TargetHorizontalTiltAngle().UUID
      ) {
        propertyChanged = "value2";
      }
  ...
  setCharacteristicValue(
    ...
    if (context !== "fromFibaro" && context !== "fromSetValue") {
      ...
      this.log(
        "Setting value to device: ",
        `${d}  parameter: ${characteristic.displayName}`
      );
      if (this.setFunctions) {
        let setFunction = this.setFunctions.setFunctionsMapping.get(
          characteristic.UUID
        );
        if (setFunction)
          setFunction.call(
  ...
  getCharacteristicValue(callback, characteristic, service, IDs) {
    this.log(
      "Getting value from device: ",
      `${IDs[0]}  parameter: ${characteristic.displayName}`
    );
    // Manage security system status
    if (service.isSecuritySystem) {
      if (!this.fibaroClient) return;
      this.fibaroClient
        .getGlobalVariable("SecuritySystem")
        .then((securitySystemStatus) => {
          if (this.getFunctions)
            this.getFunctions.getSecuritySystemState(
              callback,
              characteristic,
              service,
              IDs,
        ...
        .catch((err) => {
          this.log(
            "There was a problem getting value from Global Variable: SecuritySystem",
            ` - Err: ${err}`
          );
          callback(err, null);
        });
    ...
    if (service.isGlobalVariableSwitch) {
      if (!this.fibaroClient) return;
      this.fibaroClient
        .getGlobalVariable(IDs[1])
        .then((switchStatus) => {
          if (this.getFunctions)
            this.getFunctions.getBool(
              callback,
              characteristic,
              service,
              IDs,
        ...
        .catch((err) => {
          this.log(
            "There was a problem getting value from Global Variable: ",
            `${IDs[1]} - Err: ${err}`
          );
          callback(err, null);
        });
    ...
    let getFunction = this.getFunctions.getFunctionsMapping.get(
      characteristic.UUID
    );
    if (!getFunction) {
      callback(undefined, characteristic.value);
      ...
      this.fibaroClient
        ...
        .then((properties: any) => {
          if (getFunction.function) {
            ...
            callback(
              `No get function defined for: ${characteristic.displayName}`,
              null
            );
        })
        .catch((err) => {
  ...
  notifyIFTTT(e, val1, val2, val3) {
    ...
    request(
      ...
      function (err, response) {
        if (err) {
          that.log(
            "There was a problem sending event: ",
            `${e}, to: ${that.config.IFTTTmakerkey} - Err: ${err}`
          );
        } else {
          that.log(
            "Sent event: ",
            `${e}, to: ${that.config.IFTTTmakerkey}, for ${val1}`
          );
        }
      }
    );
