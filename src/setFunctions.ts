...
export class SetFunctions {
  ...
  constructor(hapCharacteristic, platform) {
    ...
    this.getTargetSecuritySystemSceneMapping = new Map([
      [
        this.hapCharacteristic.SecuritySystemTargetState.AWAY_ARM,
        this.platform.securitySystemScenes.SetAwayArmed,
      ],
      [
        this.hapCharacteristic.SecuritySystemTargetState.DISARM,
        this.platform.securitySystemScenes.SetDisarmed,
      ],
      [
        this.hapCharacteristic.SecuritySystemTargetState.NIGHT_ARM,
        this.platform.securitySystemScenes.SetNightArmed,
      ],
      [
        this.hapCharacteristic.SecuritySystemTargetState.STAY_ARM,
        this.platform.securitySystemScenes.SetStayArmed,
      ],
    ]);
  }
  ...
  setOn(value, callback, context, characteristic, service, IDs) {
    if (service.isVirtual && !service.isGlobalVariableSwitch) {
      ...
      if (
        (characteristic.value == true && value == 0) ||
        (characteristic.value == false && value == 1)
      )
  ...
  setLockTargetState(value, callback, context, characteristic, service, IDs) {
    if (service.isLockSwitch) {
      var action =
        value == this.hapCharacteristic.LockTargetState.UNSECURED
          ? "turnOn"
          : "turnOff";
      this.command(action, null, service, IDs);
      let lockCurrentStateCharacteristic = service.getCharacteristic(
        this.hapCharacteristic.LockCurrentState
      );
      lockCurrentStateCharacteristic.updateValue(
        value,
        undefined,
        "fromSetValue"
      );
  ...
  setTargetDoorState(value, callback, context, characteristic, service, IDs) {
    ...
    setTimeout(() => {
      ...
      let currentDoorStateCharacteristic = service.getCharacteristic(
        this.hapCharacteristic.CurrentDoorState
      );
  ...
  setTargetHeatingCoolingState(
    ...
    if (service.operatingModeId) {
      ...
      if (v != "0") {
        // set subset mode on the temperature controller ...
      ...
      if (this.platform.config.enablecoolingstatemanagemnt == "on") {
        ...
        this.command(
          "setTime",
          [0 + Math.trunc(new Date().getTime() / 1000)],
          service,
          IDs
        );
        ...
        setTimeout(() => {
          characteristic.setValue(value, undefined, "fromSetValue");
          // set also current state
          let currentHeatingCoolingStateCharacteristic = service.getCharacteristic(
            this.hapCharacteristic.CurrentHeatingCoolingState
          );
          currentHeatingCoolingStateCharacteristic.setValue(
            value,
            undefined,
            "fromSetValue"
          );
    ...
    if (Math.abs(value - characteristic.value) >= 0.5) {
      ...
      if (service.operatingModeId) {
        ...
        this.command(
          "setTime",
          [
            parseInt(this.platform.config.thermostattimeout) +
  ...
  updateHomeCenterColorFromHomeKit(h, s, v, service) {
    ...
    if (v != null) service.HSBValue.brightness = v;
    var rgb = this.HSVtoRGB(
      service.HSBValue.hue,
      service.HSBValue.saturation,
      service.HSBValue.brightness
    );
    ...
    switch (--service.countColorCharacteristics) {
      case 1:
        service.timeoutIdColorCharacteristics = setTimeout(() => {
          if (service.countColorCharacteristics < 1) return;
  ...
  command(c, value, service, IDs) {
    this.platform.fibaroClient
      .executeDeviceAction(IDs[0], c, value)
      .then((response) => {
        this.platform.log(
          "Command: ",
          c +
        ...
        if (
          this.platform.config.enableIFTTTnotification == "all" ||
          this.platform.config.enableIFTTTnotification == "hk"
        )
          this.platform.notifyIFTTT(VALUE_SET, IDs[0], c, value);
      })
      .catch((err, response) => {
        this.platform.log(
          "There was a problem sending command ",
          c + " to " + IDs[0]
        );
      });
  }
  ...
  setGlobalVariable(variableID, value) {
    this.platform.fibaroClient
      ...
      .catch((err, response) => {
        this.platform.log(
          "There was a problem setting variable: ",
          `${variableID} to ${value}`
        );
      });
  }
    ...
    this.platform.fibaroClient
      ...
      .then((properties) => {
        ...
        if (currentValue != value) {
          this.platform.log(
            "There was a problem setting value to Lock: ",
            `${IDs[0]}`
          );
